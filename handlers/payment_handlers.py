...
async def success_payment(msg: Message, session: DataInteraction, state: FSMContext, scheduler: AsyncIOScheduler, dialog_manager: DialogManager):
    mounts = int(msg.successful_payment.invoice_payload)
    await msg.delete()
    await execute_rate(msg.from_user.id, msg.bot, session, scheduler, mounts)
    if dialog_manager.has_context():
        ...
        while dialog_manager.has_context():
            ...
            try:
                ...
                ...
            counter += 1
    await dialog_manager.start(states.SubSG.start, mode=StartMode.RESET_STACK)
